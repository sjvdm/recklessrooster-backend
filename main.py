...
def fetch_coordinates():
    ...
    return [(row.gbifid, row.decimallatitude, row.decimallongitude) for row in query_job.result()]
...
def process_data():
    ...
    results = []
    for gbifid, lat, lon in coordinates:
        distance = query_overpass(lat, lon)
        results.append((gbifid, lat, lon, distance))
    return results
...
def sync_to_bigquery(results):
    ...
    rows_to_insert = [
        {"gbifid": gbifid, "latitude": lat, "longitude": lon, "distance": dist}
        for gbifid, lat, lon, dist in results
    ]
